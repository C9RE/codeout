...
// Aggregate per-session cap: the per-request caps above bound ONE POST, but nothing stopped a
// caller looping requests to fill the host disk. Bound the total bytes held in one session's
// upload dir; a new upload is refused once the dir is already at/over the cap.
const MAX_UPLOAD_TOTAL = Number(process.env.COCKPIT_MAX_UPLOAD_TOTAL) || 500 * 1024 * 1024; // 500 MB/session
...
function persist() {
	try {
		...
		// Atomic (tmp + rename) so a crash mid-write can't leave a truncated sessions.json that
		// restoreSessions() would read as empty and silently fail to reattach every session.
...
// The shell inherits only a safe, minimal environment, never the daemon's full process.env, so
// host secrets don't leak into every session. The exact whitelist + PATH handling is per-OS and
// lives in the platform layer (posix.js / windows.js); agents read their own creds from $HOME.
function childEnv() {
	return platform.buildChildEnv();
}

// How a terminal session is spawned + torn down is OS-specific (dtach on POSIX, node-pty
// direct on Windows) and lives in the platform layer. CHAT MODE never reaches that path: it
// runs the managed agent backend, not a PTY.
...
function wireChat(s, fresh) {
	...
	// `working` alone cannot tell a client the difference between "the agent is
	// thinking" and "the agent is blocked on you": a permission prompt happens
	// MID-turn, so turnLive stays true while nothing moves until you tap. Mirror the
	// pending count onto the session so /api/sessions can say which it is. Every
	// mutation of pendingPermissions goes through this, so the two cannot drift.
	const syncAwaitingInput = () => { s.awaitingInput = pendingPermissions.size > 0; };
	syncAwaitingInput();   // always a boolean, never undefined, even before the first prompt
	...
	// Union across agents (Claude: low..max; Codex: none|minimal|low|medium|high|xhigh; Gemini: n/a).
	// The chosen level rides the backend relaunch; an agent that doesn't support a value ignores it.
	...
	s.handleUserInput = (text, senderId, senderName, attachments, clientId) => {
		...
		// Applies to any CHAT-backed agent (else typing /clear would send the literal text to the
		// model). /model /effort /clear all route through relaunchBackend, which is agent-agnostic.
		// (Slash built-ins never carry attachments - they're typed commands.)
		if (CHAT_BACKENDS[s.agent] && atts.length === 0) {
...
function wireTerminal(s, fresh) {
	const pty = platform.spawnTerminal({ socket: s.socket, agent: s.agent, fresh, ptyOpts: ptyOpts(s.cwd) });
	s.pty = pty;
	s.buffer = [];
	let bufSize = 0;
	attachBroadcaster(s);
...
async function maybeNotify(s, ev) {
	try {
		...
		for (const t of pushTargets(exclude)) {
			...
			else if (r && !r.ok) console.warn('[codeout] push not delivered:', r.reason || r.status || 'unknown'); // surface misconfig (bad topic/key/JWT) instead of silently never notifying
		}
	} catch (e) {
...
export function restoreSessions() {
	...
	for (const m of Array.isArray(meta) ? meta : []) {
		...
		// Terminal sessions only survive a restart where the platform keeps a detached master
		// (POSIX + dtach). Elsewhere (Windows, or POSIX without dtach) there's no socket to
		// reattach, so skip — the platform flag makes that intent explicit.
		if (!platform.terminalSurvivesRestart || !m?.socket || !existsSync(m.socket)) continue;
		/** @type {Session} */
...
export const list = () =>
	[...sessions.values()]
		.map((s) => ({ id: s.id, cwd: s.cwd, agent: s.agent, name: s.name ?? null, avatar: s.avatar ?? null, created: s.created, idleMs: Date.now() - (s.lastOutput || s.created), chatMode: !!s.chatMode, permissionMode: s.permissionMode ?? null, working: !!s.turnLive, awaitingInput: !!s.awaitingInput, lastSeq: s.chatLog ? s.chatLog.lastSeq() : null }))
...
export function kill(id) {
	...
	// Terminal: end the agent (not just our client) — platform-specific (kill the dtach master
	// on POSIX, the ConPTY child on Windows). onExit then drops the socket + client.
	platform.killTerminal(s);
	sessions.delete(id);
...
function handleUpload(req, res, id, cors) {
	...
	// Aggregate per-session quota: refuse if this session's upload dir is already at the cap, so a
	// caller can't fill the disk across many requests (each request is still file/size-capped too).
	try {
		let used = 0;
		for (const f of readdirSync(dir)) { try { used += statSync(join(dir, f)).size; } catch { /* vanished */ } }
		if (used >= MAX_UPLOAD_TOTAL) return done(413, { error: 'upload quota for this session is full' });
	} catch { /* dir unreadable → treat as empty and allow */ }
	let bb;
	try {
...
export async function handleApi(req, res) {
	...
	try {
		...
		if (req.method === 'GET' && url.pathname === '/api/projects') {
			...
			// ----- archive lifecycle (chat sessions only) -----
			...
			if (req.method === 'GET' && arcOne) {
				const meta = readArchiveMeta(decodeURIComponent(arcOne[1]));
