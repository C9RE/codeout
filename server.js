...
import { isTunnelRequest, isLocalRequest } from './auth.js';
...
function serveStatic(req, res) {
	...
	if ((isTunnelRequest(req) || !isLocalRequest(req)) && u.searchParams.has('token')) {
...
// Process-level backstop: a single throwing request/connection handler must NOT take the
// whole daemon down (and every live session with it). Node exits on an unhandled rejection
// by default; a long-lived self-hosted daemon should log and stay up instead — systemd
// Restart=always is only a second net, and `npx`/bare `node` starts have none.
let guardsInstalled = false;
function installProcessGuards() {
	if (guardsInstalled) return;
	guardsInstalled = true;
	process.on('unhandledRejection', (reason) => {
		console.error('[codeout] unhandled rejection (daemon kept alive):', reason?.stack ?? reason?.message ?? reason);
	});
	process.on('uncaughtException', (err) => {
		console.error('[codeout] uncaught exception (daemon kept alive):', err?.stack ?? err?.message ?? err);
	});
}
...
export function startDaemon({ port = Number(process.env.PORT) || 8400, host = process.env.HOST || '127.0.0.1' } = {}) {
	...
	const server = createServer((req, res) => {
		// Never let a throw/rejection in a handler escape to the process (that would crash the
		// daemon for every connected device); turn it into a clean 500 for this one request.
		Promise.resolve()
			.then(() => handleApi(req, res)) // session API (shared map)
			.then((handled) => { if (!handled) serveStatic(req, res); }) // else -> static web UI (SPA)
			.catch((e) => {
				console.error('[codeout] request handler error:', e?.message ?? e);
				try { if (!res.headersSent) res.writeHead(500, { 'content-type': 'application/json' }); res.end('{"error":"server error"}'); }
