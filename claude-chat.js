...
export function startClaudeChat({ cwd, env, resumeId, model, effort, permissionMode, extraSystemPrompt, emit: rawEmit, onSessionId, onSlashCommands, onMeta, onPermission }) {
	...
	// windowsHide keeps a console window from flashing when the agent spawns on Windows; it's a
	// no-op on macOS/Linux. On Windows a native `claude.exe` (WinGet/installer) resolves on PATH;
	// the .ps1 installer sets that up so a bare spawn just works.
