...
export function buildChildEnv() {
	...
	// `claude` installs to ~/.local/bin and `gemini` (bun i -g) to ~/.bun/bin, both of which a
	// systemd --user service's PATH omits, so a bare spawn() would ENOENT. Add the usual bin dirs
	// so the agent CLIs are findable.
	const parts = env.PATH ? env.PATH.split(':') : [];
	for (const p of [join(homedir(), '.local/bin'), join(homedir(), '.bun/bin'), '/usr/local/bin', '/usr/bin', '/bin']) {
