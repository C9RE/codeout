...
/**
 * Did this request originate from a trusted local network — loopback, RFC1918 LAN, CGNAT
 * (Tailscale 100.64/10), or link-local? The owner token and the plaintext WS are accepted
 * ONLY from here, so binding 0.0.0.0 for LAN access can NEVER expose the control plane to a
 * public IP (a self-hosted VPS with no firewall). A cloudflared-tunneled request arrives from
 * loopback but carries CF headers (isTunnelRequest already rejects it); a direct public
 * connection carries a public remote address and is rejected here. Fail-safe: an unknown/empty
...
export function isLocalRequest(req) {
	...
	if (/^100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./.test(ip)) return true; // 100.64/10 CGNAT (Tailscale)
...
export function apiTokenOk(req) {
	// Owner token: only over the tunnel is rejected AND only from a local network is accepted, so
	// a public direct hit on a 0.0.0.0-bound daemon (no CF headers, public remote IP) can't use it.
	if (isTunnelRequest(req) || !isLocalRequest(req)) return false;
