...
## Sessions list: `working` + `awaitingInput` + `lastSeq`

`GET /api/sessions` items carry three chat-status fields: `working` (a turn is live right
now — drive a "busy" indicator), `awaitingInput` (a tool-permission prompt is outstanding,
so the agent is blocked on the USER, not thinking) and `lastSeq` (newest retained chat seq,
`null` for terminal sessions).

**`awaitingInput` implies `working`.** A permission prompt happens mid-turn, so `working`
stays true the whole time the agent sits blocked. That is why the two fields are separate:
on `working` alone a session waiting for a tap is indistinguishable from one that is
genuinely generating, and clients rendered a "busy" animation on a session that actually
needed the user. Clients should check `awaitingInput` FIRST and give it its own state
(something that reads as "needs you"), falling back to the busy state when it is false.
It is always a boolean, and it cannot be stranded true: turn end and backend death both
deny every outstanding prompt and clear it. A client that stores the last seq it SAW per session can badge unread
...
- **Gemini** (`gemini-chat.js`, **RETIRED 2026-07-15** — Gemini is now a **terminal** agent that
  runs `agy` (Antigravity, `AGENTS` in sessions.js), not a chat backend. Google severed the gemini
  CLI from consumer subs on 2026-06-18 (`UNSUPPORTED_CLIENT`), and its successor `agy` has no ACP
  protocol, so native chat on the AI Pro sub is impossible. `gemini-chat.js` is kept for revert /
  the day Google reopens it. The retired design is documented below.): `gemini --acp` (Agent Client Protocol) on the same
